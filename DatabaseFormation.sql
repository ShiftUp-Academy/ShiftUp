...
CREATE TABLE ProgrammeFormation (
    ...
    Type VARCHAR(50) DEFAULT 'Formation' CHECK (Type IN ('Formation', 'Seminaire')),
    Titre VARCHAR(155) NOT NULL,
    LienPhoto TEXT,
    ApercuVideo TEXT,
    Statut VARCHAR(50) DEFAULT 'Dépublié' CHECK (Statut IN ('Dépublié', 'Publié')),
    StatutVerrouillageProgression VARCHAR(50) DEFAULT 'Verrouillé' CHECK (StatutVerrouillageProgression IN ('Verrouillé', 'Déverrouillé', 'Libre')),
    Descriptions TEXT,
    DateSeminaire DATE,
    HeureSeminaire VARCHAR(50),
    LieuSeminaire VARCHAR(255),
    NombreDeJours INTEGER,
    ModaliteSeminaire VARCHAR(50) DEFAULT 'Présentiel' CHECK (ModaliteSeminaire IN ('En ligne', 'Présentiel')),
    LienGoogleMeet TEXT,
    Prix DECIMAL(10, 2),
...
CREATE TABLE Lecons (
    ...
    SourceType VARCHAR(255) CHECK (SourceType IN ('Youtube', 'Cloudinary', 'Local', 'Lien_Externe', 'Interne')),
    Contenu TEXT,
    DelaiDrop INT,
    NombreDePages INT,
    Ordre INT DEFAULT 0,
